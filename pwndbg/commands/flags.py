...
def setflag(flag, value):
    ...
    register_set = pwndbg.gdb.regs.arch_to_regs[pwndbg.gdb.arch.current]

    flag = flag.upper()
    for flag_reg, flags in register_set.flags.items():
        for (flag_name, flag_bit) in flags.items():
            if flag_name == flag:
                old_flags_reg_value = pwndbg.gdb.regs[flag_reg]
                ...
                setattr(pwndbg.gdb.regs, flag_reg, new_flags_reg_value)
                print(
