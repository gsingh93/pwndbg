    ...
    def set(self, const, value):
        ...
        if name in ["AT_EXECFN", "AT_PLATFORM"]:
            try:
                ...
                value = value.cast(pwndbg.gdb.typeinfo.pchar)
                value = value.string()
            except Exception:
...
def use_info_auxv():
    lines = pwndbg.gdb.info.auxv().splitlines()
...
def find_stack_boundary(addr):
    # For real binaries, we can just use pwndbg.gdb.memory.find_upper_boundary
    ...
    try:
        while True:
            if b"\x7fELF" == pwndbg.gdb.memory.read(addr, 4):
                break
            addr += pwndbg.gdb.memory.PAGE_SIZE
...
def walk_stack():
    if not pwndbg.gdb.abi.linux:
        return None
    if pwndbg.gdb.qemu.is_qemu_kernel():
        return None
...
def walk_stack2(offset=0):
    ...
    p = gdb.Value(end).cast(pwndbg.gdb.typeinfo.ulong.pointer())
    ...
    auxv = AUXV()
    while True:
        const = int((p + 0).dereference()) & pwndbg.gdb.arch.ptrmask
        value = int((p + 1).dereference()) & pwndbg.gdb.arch.ptrmask
...
def _get_execfn():
    ...
    if not pwndbg.gdb.memory.peek(pwndbg.gdb.regs.sp):
        return
    ...
    addr = pwndbg.gdb.stack.find_upper_stack_boundary(pwndbg.gdb.regs.sp)

    while pwndbg.gdb.memory.byte(addr - 1) == 0:
        addr -= 1

    while pwndbg.gdb.memory.byte(addr - 1) != 0:
        addr -= 1

    v = pwndbg.gdb.strings.get(addr, 1024)
    if v:
