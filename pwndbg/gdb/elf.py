...
def read(typ, address, blob=None):
    ...
    if not blob:
        data = pwndbg.gdb.memory.read(address, size)
    else:
...
def entry():
    ...
    """
    entry = pwndbg.gdb.auxv.get().AT_ENTRY
    if entry:
        return entry

    # Looking for this line:
    # Entry point: 0x400090
    for line in pwndbg.gdb.info.files().splitlines():
    ...
    for name in ["_start", "start", "__start", "main"]:
        try:
            return pwndbg.gdb.symbol.address(name)
        except gdb.error:
            pass
...
def get_ehdr(pointer):
    ...
    vmmap = pwndbg.gdb.vmmap.find(pointer)
    base = None
    ...
    if pwndbg.gdb.memory.read(vmmap.start, 4) == b"\x7fELF":
        base = vmmap.start
...
def map_inner(ei_class, ehdr, objfile):
    ...
    for phdr in iter_phdrs(ehdr):
        ...
        for page_addr in range(vaddr, vaddr + memsz, pwndbg.gdb.memory.PAGE_SIZE):
            if page_addr in pages:
                ...
                page = pwndbg.lib.memory.Page(
                    page_addr, pwndbg.gdb.memory.PAGE_SIZE, flags, offset + (page_addr - vaddr)
                )
                pages.append(page)
