...
def get_version():
    if pwndbg.heap.current.libc_has_debug_syms():
        addr = pwndbg.gdb.symbol.address(b"__libc_version")
        if addr is not None:
            ver = pwndbg.gdb.memory.string(addr)
            return tuple([int(_) for _ in ver.split(b".")])
    for addr in pwndbg.gdb.search.search(b"GNU C Library"):
        banner = pwndbg.gdb.memory.string(addr)
