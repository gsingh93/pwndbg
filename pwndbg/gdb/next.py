...
def next_int(address=None):
    ...
    """
    if address is None:
        ins = pwndbg.disasm.one(pwndbg.gdb.regs.pc)
        if not ins:
            return None
        address = ins.next
...
def break_on_program_code():
    ...
    """
    mp = pwndbg.gdb.proc.mem_page
    start = mp.start
    end = mp.end

    if start <= pwndbg.gdb.regs.pc < end:
...
def break_on_next(address=None):
    address = address or pwndbg.gdb.regs.pc
    ins = pwndbg.disasm.one(address)
