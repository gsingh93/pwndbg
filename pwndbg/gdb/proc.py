...
class module(ModuleType):
    ...
    def exe(self):
        ...
        2. gdb -ex "target remote :1234" -ex "pi pwndbg.gdb.proc.exe"

        If you need to process the debugged file use:
            `pwndbg.file.get_file(pwndbg.gdb.proc.exe)`
        """
        return gdb.current_progspace().filename

    @property
    def mem_page(self):
        return next(p for p in pwndbg.gdb.vmmap.get() if p.objfile == self.exe)
