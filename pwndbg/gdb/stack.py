...
def find_upper_stack_boundary(stack_ptr, max_pages=1024):
    ...
    if not pwndbg.gdb.abi.linux:
        return stack_ptr + pwndbg.gdb.memory.PAGE_SIZE

    return pwndbg.gdb.memory.find_upper_boundary(stack_ptr, max_pages)
...
def current():
    ...
    """
    return find(pwndbg.gdb.regs.sp)
...
def is_executable():
    ...
    ehdr = pwndbg.gdb.elf.exe()

    for phdr in pwndbg.gdb.elf.iter_phdrs(ehdr):
