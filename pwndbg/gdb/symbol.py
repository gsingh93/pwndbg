...
def autofetch():
    """ """
    global remote_files_dir
    if not pwndbg.gdb.remote.is_remote():
        return

    if pwndbg.gdb.qemu.is_qemu_usermode():
        return

    if pwndbg.gdb.android.is_android():
        return
    ...
    for mapping in pwndbg.gdb.vmmap.get():
...
def get(address, gdb_only=False):
    ...
    """
    # Fast path
    if address < pwndbg.gdb.memory.MMAP_MIN_ADDR or address >= ((1 << 64) - 1):
        return ""

    # Don't look up stack addresses
    if pwndbg.gdb.stack.find(address):
        return ""
    ...
    if not gdb_only and result.startswith("No symbol"):
        address = int(address)
        exe = pwndbg.gdb.elf.exe()
        if exe:
            exe_map = pwndbg.gdb.vmmap.find(exe.address)
...
def address(symbol, allow_unmapped=False):
    ...
    try:
        ...
        if not allow_unmapped and not pwndbg.gdb.vmmap.find(address):
            return None
...
def add_main_exe_to_symbols():
    ...
    mmap = pwndbg.gdb.vmmap.find(addr)
    if not mmap:
        return
