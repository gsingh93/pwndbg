...
def get():
    ...
    if (
        not pages
        and pwndbg.gdb.qemu.is_qemu_kernel()
    ...
    if not pages:
        ...
        if pages:
            ...
            if pwndbg.gdb.qemu.is_qemu():
                return (pwndbg.lib.memory.Page(0, pwndbg.gdb.arch.ptrmask, 7, 0, "[qemu]"),)
            pages.extend(info_files())

        pages.extend(pwndbg.gdb.stack.stacks.values())
...
def explore(address_maybe):
    ...
    flags = 4 if pwndbg.gdb.memory.peek(address_maybe) else 0

    if not flags:
        return None

    flags |= 2 if pwndbg.gdb.memory.poke(address_maybe) else 0
    flags |= 1 if not pwndbg.gdb.stack.nx else 0
...
def explore_registers():
    for regname in pwndbg.gdb.regs.common:
        find(pwndbg.gdb.regs[regname])
...
def coredump_maps():
    ...
    # TODO/FIXME: Can we uxe `pwndbg.gdb.auxv.get()` for this somehow?
...
def info_sharedlibrary():
    ...
    for line in gdb.execute("info sharedlibrary", to_string=True).splitlines():
        ...
        pages.extend(pwndbg.gdb.elf.map(text, obj))
...
def info_files():
    ...
    for line in gdb.execute("info files", to_string=True).splitlines():
        ...
        pages.extend(pwndbg.gdb.elf.map(vaddr, objfile))
...
def info_auxv(skip_exe=False):
    ...
    if not skip_exe and (entry or phdr):
        pages.extend(pwndbg.gdb.elf.map(entry or phdr, exe_name))

    if base:
        pages.extend(pwndbg.gdb.elf.map(base, "[linker]"))

    if vdso:
        pages.extend(pwndbg.gdb.elf.map(vdso, "[vdso]"))
...
def find_boundaries(addr, name="", min=0):
    ...
    which are mapped.
    """
    start = pwndbg.gdb.memory.find_lower_boundary(addr)
    end = pwndbg.gdb.memory.find_upper_boundary(addr)
...
def check_aslr():
    ...
    if pwndbg.gdb.proc.alive:
        try:
            data = pwndbg.file.get("/proc/%i/personality" % pwndbg.gdb.proc.pid)
...
def mark_pc_as_executable():
    mapping = find(pwndbg.gdb.regs.pc)
